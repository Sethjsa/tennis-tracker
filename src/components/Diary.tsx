...
import { parseScore, roundLabel, roundRank } from "@/lib/score";
...
export default function Diary() {
    ...
    async (inst: TournamentInstance) => {
      ...
      const [{ data: rows }, { data: saved }] = await Promise.all([
        supabase
          ...
          .select("*")
          .eq("tour", inst.tour)
          .eq("tourney_name", inst.tourney_name)
          .eq("year", inst.year),
        supabase
          .from("saved_matches")
          .select("source_match_id")
          .eq("tournament", inst.tourney_name)
          .eq("year", inst.year),
      ]);

      const list = ((rows ?? []) as TourMatch[]).sort(
        (a, b) => roundRank(a.round) - roundRank(b.round) || (a.match_num ?? 0) - (b.match_num ?? 0),
      );
      setMatches(list);
  ...
  const visibleMatches = useMemo(
    () => matches.filter((m) => selectedRounds.has(m.round ?? "?")),
  ...
  async function save() {
    ...
    const rows = [...picks.entries()].map(([id, sel]) => {
      ...
      const b = parseScore(m.score, m.best_of);
      return {
        user_id: userId,
        source_match_id: m.id,
        tour: m.tour,
        player1: m.winner_name,
        player2: m.loser_name,
        ...
        best_of: m.best_of,
        minutes: sel.minutes,
        winner_ioc: m.winner_ioc,
        loser_ioc: m.loser_ioc,
        winner_rank: m.winner_rank,
        loser_rank: m.loser_rank,
        sets_total: b.setsTotal,
        games_total: b.gamesTotal,
        tiebreaks_total: b.tiebreaksTotal,
        went_distance: b.wentDistance,
        straight_sets: b.straightSets,
        ...
        <div className="space-y-2">
          {instances.length === 0 && !searching && (
            <p className="text-sm text-neutral-500">
              Search for a tournament to begin. Tip: leave the year blank to see every edition.
            </p>
          )}
          {instances.map((inst) => (
            <button
              key={`${inst.tour}-${inst.tourney_name}-${inst.year}`}
            ...
            >
              <div>
                <div className="font-medium">
                  {inst.tourney_name}{" "}
                  <span className="text-neutral-400">{inst.year}</span>
                </div>
                <div className="mt-0.5 flex items-center gap-2 text-xs text-neutral-500">
                  <span className="uppercase">{inst.tour}</span>
                  {inst.surface && (
                    <span className={`rounded px-1.5 py-0.5 ${surfaceClass(inst.surface)}`}>
                      {inst.surface}
                    </span>
                  )}
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="font-medium">
                            {m.winner_name}{" "}
                            <span className="text-neutral-400">def.</span> {m.loser_name}
                          </div>
                          ...
                          {pick && (
                            <div className="mt-3 flex flex-wrap items-center gap-3">
                              <Stars
                                value={pick.rating}
                                onChange={(v) => updatePick(m.id, { rating: v })}
                              />
                              <select
                                value={pick.surface ?? ""}
                                onChange={(e) =>
                                  updatePick(m.id, { surface: e.target.value || null })
                                }
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  updatePick(m.id, {
                                    minutes: e.target.value ? parseInt(e.target.value, 10) : null,
                                  })
                                }
          ...
          {picks.size > 0 && (
            <div className="sticky bottom-4 flex items-center justify-between gap-3 rounded-xl border border-green-500 bg-white dark:bg-neutral-900 px-4 py-3 shadow-lg">
              <span className="text-sm font-medium">
                {picks.size} match{picks.size === 1 ? "" : "es"} selected
              </span>
              <button
                onClick={save}
                disabled={saving}
